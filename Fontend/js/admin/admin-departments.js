// ================================================================
// ADMIN DEPARTMENTS - Quản lý phòng ban (không bao gồm Team)
// ================================================================
...
function renderDepartmentsTab() {
    ...
    let html = `
        ...
        <div id="dept-sub-content" style="display:grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap:16px;">
    `;

    if (!departments || departments.length === 0) {
        html += `
            <div style="grid-column:1/-1; padding:30px; text-align:center; background:#f8fafc; border-radius:8px; border:1px solid #e2e8f0;">
                <i class="fas fa-building" style="font-size:40px; color:#ccc; display:block; margin-bottom:12px;"></i>
                <div style="font-size:16px; color:#666;">Chưa có phòng ban nào</div>
                <div style="font-size:13px; color:#999; margin-top:4px;">Bấm "Thêm phòng ban" để tạo mới.</div>
            </div>
        `;
    } else {
        ...
        if (filtered.length === 0 && rootDepts.length > 0) {
            html += `<div style="grid-column:1/-1; text-align:center; color:#999; padding:20px;">Không tìm thấy phòng ban</div>`;
        }
    }
...
function renderDepartmentCard(dept, departments, users, isSub = false) {
    ...
    const indent = isSub ? 'margin-left: 12px; border-left: 3px solid #3498db; padding-left: 12px;' : '';
...
console.log('✅ Admin Departments module updated (teams removed).');