...
function getPRActions(pr) {
    const user = getUser();
    let actions = '';
    const canEdit = (user.role === 'ADMIN') || (user.id === pr.createdBy && (pr.status === 'PENDING' || pr.status === 'DRAFT'));
    ...
    if (pr.status === 'PENDING') {
        ...
        if (user.role === roleCanApprove) {
            actions += ` <button class="btn btn-success btn-sm" onclick="approvePR(${pr.id})">Duyệt</button> <button class="btn btn-danger btn-sm" onclick="rejectPR(${pr.id})">Từ chối</button>`;
        }
    }
    if (pr.status === 'APPROVED' && user.role === 'PURCHASING') {
...
// ====== TẠO PR MỚI ======
document.getElementById('btn-create-pr')?.addEventListener('click', function() {
    showCreatePRModal();
});
...
async function editPR(id) {
    try {
        ...
        if (pr.status !== 'DRAFT' && pr.status !== 'PENDING') {
            showWarning('Chỉ có thể sửa PR ở trạng thái DRAFT hoặc PENDING');
            return;
        }
